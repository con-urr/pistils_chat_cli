...
requireLiveSmokeOptIn('smoke-supervisor-live-reply');
...
const home = path.join(os.tmpdir(), `agenttalk-supervisor-live-reply-${suffix}`);
const senderStateDir = path.join(home, 'sender');
const targetHandle = `reply-target-${suffix}`.toLowerCase().replace(/[^a-z0-9_-]/g, '-').slice(0, 62);
const senderHandle = `reply-sender-${suffix}`.toLowerCase().replace(/[^a-z0-9_-]/g, '-').slice(0, 62);
...
const connectorPath = path.join(home, 'reply-connector.mjs');
const expectedReply = `supervisor live reply ${suffix}`;
await fs.writeFile(
  connectorPath,
  ...
  handled: true,
  replySent: false,
  replyText: ${JSON.stringify(expectedReply)},
  message: 'replyText returned for supervisor delivery'
}));\n`,
  'utf8'
);
...
try {
  ...
  await sender.createAccount({
    ...
    role: 'agent',
    bio: 'live supervisor reply smoke sender',
    clientRequestId: `live-supervisor-reply:${suffix}:sender`,
  });
  await sleep(1000);
  ...
  const messageRequestId = await sender.sendDirectMessage({
    ...
    text: `live reply smoke ${suffix}`,
    clientRequestId: `live-supervisor-reply:${suffix}:message`,
  });
  ...
  if (!reply) {
    throw new Error(`Expected connector reply was not visible: ${JSON.stringify(messages)}`);
  }
